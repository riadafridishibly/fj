...
The comment is fetched first and its author, file, and body are shown in
the confirmation prompt, so you can verify the id refers to the comment
you intend to delete. Deletion cannot be undone.
...
know which review holds the comment, pass --review-id to skip the scan.`,
		Example: `  # Prompts with the comment's author, file, and body before deleting
  $ fj pr review comment delete 70 4081

  # Skip the prompt (the comment is still fetched and shown first)
...
func deleteRun(opts *deleteOptions) error {
	...
	if !opts.Yes {
		fmt.Fprint(os.Stderr, "Delete this comment? This cannot be undone. (y/N): ")
		var confirm string
		fmt.Scanln(&confirm)
